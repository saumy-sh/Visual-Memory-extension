...
export function ExpandedPreview() {
  ...
  const handleNavigate = () => {
    ...
    requestAnimationFrame(() => {
      requestAnimationFrame(() => {
        ...
        const absoluteTop = screenshot.capturePosition.scrollY + screenshot.selectionRect.y;
        const elementMid = absoluteTop + screenshot.selectionRect.height / 2;
        const scrollTarget = Math.max(0, elementMid - viewportH / 2);
        window.scrollTo({ top: scrollTarget, left: screenshot.capturePosition.scrollX, behavior: "smooth" });
      });
    });
  };
  ...
  const btnStyle = (bg: string): React.CSSProperties => ({
    padding: "5px 12px", borderRadius: "6px", background: bg,
  ...
  const handle = (edge: string, style: React.CSSProperties): React.CSSProperties => ({
    ...
    cursor: edge === "e" || edge === "w" ? "ew-resize"
      ...
      : edge === "se" || edge === "nw" ? "nwse-resize" : "nesw-resize",
  });
  ...
  return (
    ...
    >
      <motion.div
        ...
        // ── no onClick stopPropagation needed, wrapper is pointer-events:none ──
        style={{
          position: "absolute",
          top: "50%",
          left: "50%",
      ...
      >
        ...
        <div style={{ flex: 1, overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
          <img src={screenshot.image} alt="Screenshot" draggable={false}
        ...
        <div style={{
          ...
          backgroundSize: "3px 3px", borderRadius: "2px",
        }} />
      </motion.div>
    </motion.div>
  );
